...
export function createCompleteShelfBookRig(config = {}) {
  ...
  const trackedTextures = [coverTexture, foilTexture, clothBumpTexture, clothSurfaceMaps.normal, clothSurfaceMaps.roughness, paperFaceTexture, ...interiorPageTextures, endpaperTexture, pageEdgeTextures.fore, pageEdgeTextures.headTail, spineTexture, spineFoilTexture, backCoverTexture, backFoilTexture, foilEmbossTexture, spineEmbossTexture, backEmbossTexture];
  ...
  const endpaperMaterial = new THREE.MeshPhysicalMaterial({ color: new THREE.Color(book.palette.paperPale).lerp(new THREE.Color(0xf2ead8), 0.5), map: endpaperTexture, bumpMap: paperFaceTexture, bumpScale: 0.0018, roughness: 0.94, metalness: 0, sheen: 0.025, sheenRoughness: 1, side: THREE.DoubleSide, transparent: true });
  ...
  const presentationApplied = applyCompleteShelfPresentation({
    presentation,
    textures: { cover: coverTexture, coverFoil: foilTexture, spine: spineTexture, spineFoil: spineFoilTexture, back: backCoverTexture, backFoil: backFoilTexture, endpaper: endpaperTexture, interiors: interiorPageTextures },
    materials: { cloth: [cloth], foil: [foilArt, spineFoilArt, backFoilArt, headbandMaterial], paper: [endpaperMaterial, pageMaterial, ...interiorPageMaterials, blankPageMaterial], edge: [foreEdgeMaterial, headTailEdgeMaterial] },
  ...
  const spineLining = createMesh(new RoundedBoxGeometry(spineWidth * 0.68, height - 0.056, Math.max(0.045, pageDepth - 0.008), 1, 0.0015), endpaperMaterial, 'codex-spine-lining'); spineLining.position.set(-width * 0.5 + spineWidth * 0.38, 0, 0); motion.add(spineLining);
