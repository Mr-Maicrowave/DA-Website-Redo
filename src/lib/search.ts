export type SearchRecord = { title: string; body?: string; category?: string; keywords?: string[]; concepts?: string[] };
...
export const scoreSearchRecords = <T extends SearchRecord>(records:T[], query:string):SearchDiagnostic<T>[] => {
  const terms=words(query).map(stem); if(!terms.length)return records.map(record=>({record,score:0,reasons:[]}));
  return records.map((record,index)=>{const title=words(record.title).map(stem), category=words(record.category).map(stem), keywords=(record.keywords??[]).flatMap(words).map(stem), body=words(record.body).map(stem); const recordConcepts=new Set(record.concepts??[]); let score=0; const reasons:string[]=[];
    for(const term of terms){const concepts=conceptsFor(term); if(title.includes(term)){score+=120;reasons.push(`title:${term}`)} else if(category.includes(term)){score+=90;reasons.push(`category:${term}`)} else if(keywords.includes(term)){score+=75;reasons.push(`keyword:${term}`)}
      if(concepts.some(c=>recordConcepts.has(c))){score+=85;reasons.push(`concept:${concepts.find(c=>recordConcepts.has(c))}`)}
      else if(concepts.length && [...title,...category,...keywords].some(token=>concepts.some(c=>conceptGroups[c].map(stem).includes(token)))){score+=65;reasons.push(`concept-field:${term}`)}
      else if([...title,...category,...keywords].some(token=>token.startsWith(term) && term.length>=4)){score+=45;reasons.push(`prefix:${term}`)}
      else if([...title,...category,...keywords].some(token=>closeTypo(term,token))){score+=30;reasons.push(`typo:${term}`)}
