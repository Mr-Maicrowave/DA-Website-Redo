    ...
    <div className="min-h-screen overflow-x-hidden pt-[120px]" style={{ background: c.white, color: c.navy, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: c.white, borderBottom: `2px solid ${c.border}`, display: 'flex', justifyContent: 'center', overflowX: 'auto' }}>
        {[
          ...
          <Link key={i} to={tab.to} style={{ padding: '14px 32px', fontSize: '0.88rem', fontWeight: 700, textDecoration: 'none', color: tab.active ? c.pinkText : c.muted, borderBottom: tab.active ? `3px solid ${c.pinkText}` : '3px solid transparent', marginBottom: -2, whiteSpace: 'nowrap', transition: 'all .2s' }}>
            {tab.label}
          </Link>
        ))}
      </div>
      ...
      {/* HERO */}
      <div style={{ background: 'linear-gradient(135deg, #fff0f8 0%, #fce7f3 35%, #f3e8ff 65%, #e8f5e9 100%)', padding: '80px 52px 72px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -60, right: -60, width: 320, height: 320, borderRadius: '50%', background: 'radial-gradient(circle, rgba(249,168,212,0.35), transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -40, left: -40, width: 260, height: 260, borderRadius: '50%', background: 'radial-gradient(circle, rgba(192,132,252,0.18), transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ display: 'inline-block', fontSize: '0.75rem', fontWeight: 800, letterSpacing: '2.5px', textTransform: 'uppercase', color: c.pinkText, background: c.white, border: `1.5px solid ${c.pinkMid}`, padding: '6px 20px', borderRadius: 999, marginBottom: 22 }}>
        ...
        <h1 style={{ fontSize: 'clamp(2.4rem, 5vw, 3.8rem)', fontWeight: 900, lineHeight: 1.1, letterSpacing: '-2px', color: c.navy, marginBottom: 18 }}>
          Build the <em style={{ fontStyle: 'normal', color: c.pinkText }}>Foundations</em><br />That Last a Lifetime
        </h1>
      ...
      <section style={{ padding: '72px 52px', maxWidth: 1140, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 44 }}>
          <div style={{ display: 'inline-block', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '2.5px', textTransform: 'uppercase', color: c.gold, background: c.goldPale, border: '1px solid #e8d280', padding: '5px 16px', borderRadius: 999, marginBottom: 16 }}>Why It Matters</div>
      ...
      <section style={{ background: c.soft, borderTop: `2px solid ${c.border}`, borderBottom: `2px solid ${c.border}`, padding: '72px 52px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 36, maxWidth: 1100, margin: '0 auto', alignItems: 'start' }} className="ey-grid-responsive">
                ...
                {curriculumRows.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? c.white : c.soft }}>
                    <td style={{ padding: '13px 18px', color: c.navy, fontWeight: 700, borderBottom: `1px solid ${c.border}` }}>
                      {row.area}
                      {row.badge && <span style={{ marginLeft: 8, fontSize: '0.72rem', background: c.pink, color: c.pinkText, padding: '2px 10px', borderRadius: 999, fontWeight: 800 }}>{row.badge}</span>}
                    </td>
          ...
          <div style={{ background: c.white, border: `2px solid ${c.border}`, borderTop: `5px solid ${c.pinkMid}`, borderRadius: 24, padding: '36px 30px', boxShadow: '0 8px 32px rgba(12,31,74,0.08)' }}>
            <div style={{ color: c.goldLight, fontSize: '1.2rem', letterSpacing: 4, marginBottom: 14 }}>★★★★★</div>
            <div style={{ fontSize: '2.8rem', lineHeight: 1, color: c.pinkMid, marginBottom: 6 }}>"</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
              <div style={{ width: 48, height: 48, borderRadius: '50%', background: s.avatarBg, border: `2.5px solid ${s.avatarBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: s.avatarText, fontSize: '1.1rem' }}>{s.initial}</div>
              <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <button onClick={() => setSlide(i => (i - 1 + slides.length) % slides.length)} style={{ background: c.pink, border: 'none', borderRadius: '50%', width: 34, height: 34, cursor: 'pointer', fontSize: '1rem', color: c.pinkText, fontWeight: 900 }}>‹</button>
              <div style={{ display: 'flex', gap: 7 }}>
                {slides.map((_, i) => (
                  <button key={i} onClick={() => setSlide(i)} style={{ width: i === slide ? 22 : 9, height: 9, borderRadius: 4, border: 'none', background: i === slide ? c.pinkMid : c.border, cursor: 'pointer', padding: 0, transition: 'all .2s' }} />
                ))}
              </div>
              <button onClick={() => setSlide(i => (i + 1) % slides.length)} style={{ background: c.pink, border: 'none', borderRadius: '50%', width: 34, height: 34, cursor: 'pointer', fontSize: '1rem', color: c.pinkText, fontWeight: 900 }}>›</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '72px 52px', maxWidth: 1140, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 22 }}>
          {approachSteps.map((step, i) => (
            <div key={i} style={{ background: c.white, border: `2px solid ${c.border}`, borderRadius: 20, padding: '32px 28px', boxShadow: '0 2px 12px rgba(12,31,74,0.05)', transition: 'all .25s' }}
      ...
      {/* FIT */}
      <div style={{ background: c.pink, borderTop: `2px solid ${c.pinkMid}`, borderBottom: `2px solid ${c.pinkMid}` }}>
        <div style={{ padding: '72px 52px', maxWidth: 900, margin: '0 auto' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 36 }}>
            {fitPoints.map((pt, i) => (
              <li key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', background: c.white, border: `1.5px solid ${c.pinkMid}`, borderRadius: 14, padding: '18px 22px' }}>
      ...
      <style>{`@media(max-width:768px){.ey-grid-responsive{grid-template-columns:1fr!important;}}`}</style>
      <FooterNew />
    </div>
  );
};
