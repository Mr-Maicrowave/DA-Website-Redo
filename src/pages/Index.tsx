...
import { resetBookIntroComplete, shouldShowBookIntro } from '@/lib/bookIntroSession';
...
import { AcademyMap } from '@/components/home/AcademyMap';
import { SuccessStoriesSection, type SuccessStory } from '@/components/home/SuccessStoriesSection';
import { MentorsSection } from '@/components/home/MentorsSection';
import { WhyFamiliesStay } from '@/components/home/WhyFamiliesStay';
import { BeginJourneyCTA } from '@/components/home/BeginJourneyCTA';
import { StorybookFooter } from '@/components/home/StorybookFooter';
import '@/styles/homepage-screen-fit.css';
...
const toSuccessStoryCategory = (
  review: ReviewRecord,
): SuccessStory["category"] => {
  if (review.newTags.includes("HSC Success")) return "HSC";
  if (review.newTags.includes("Mathematics")) return "Mathematics";
  if (review.newTags.includes("English")) return "English";
  if (review.newTags.includes("Confidence")) return "Confidence";
  return "Academic Growth";
};

const toSecondarySuccessStoryCategory = (
  review: ReviewRecord,
): SuccessStory["secondaryCategory"] => {
  const primary = toSuccessStoryCategory(review);
  const candidates: SuccessStory["category"][] = [
    "HSC",
    "Mathematics",
    "English",
    "Confidence",
  ...
  return candidates.find((category) => {
    ...
    if (category === "HSC") return review.newTags.includes("HSC Success");
    return review.newTags.includes(category);
  });
};

const successStoryCards: SuccessStory[] = CAROUSEL_REVIEWS
  .filter((review) => review.id.startsWith("cr-"))
  .map((review) => ({
    id: review.id,
    studentName: review.author,
    studentDetails: `${review.yearLevel} · ${review.category}`,
    category: toSuccessStoryCategory(review),
    secondaryCategory: toSecondarySuccessStoryCategory(review),
    eyebrow: review.subject,
    transformationBefore: review.result.before,
    transformationAfter: review.result.after,
    quote: review.pullQuote,
  }));
...
// ══════════════════════════════════════════════════════════════
const Index = () => {
  const [showBookIntro, setShowBookIntro] = useState(shouldShowBookIntro);
  const [selectedSuccessReview, setSelectedSuccessReview] =
    useState<ReviewRecord | null>(null);

  const replayBookIntro = () => {
    resetBookIntroComplete();
    window.scrollTo({ top: 0, behavior: 'smooth' });
    setShowBookIntro(true);
  };

  return (
    <div id="top" style={{ fontFamily: sans, overflowX: 'clip' }}>
      <IntroVideo />
      ...
      <main>
        ...
        <DayAtDA
          images={[
            {
              src: "/images/community/class_induction.jpg",
              alt: "DA Tuition students taking part in a classroom lesson",
              position: "50% 42%",
            },
            {
              src: "/images/community/tutor_one_on_one.jpg",
              alt: "A DA Tuition tutor supporting students during class",
              position: "50% 45%",
            },
            {
              src: "/images/community/teacher_kids_warmth.jpg",
              alt: "DA Tuition students and educators sharing a positive learning moment",
              position: "50% 40%",
            },
          ]}
        />
        ...
        <SuccessStoriesSection
          stories={successStoryCards}
          onReadStory={(story) => {
            const review = CAROUSEL_REVIEWS.find(
              (candidate) => candidate.id === story.id,
            );

            if (review) {
              setSelectedSuccessReview(review);
            }
          }}
        />
      ...
      <AnimatePresence>
        {selectedSuccessReview && (
          <StoryModal
            key={selectedSuccessReview.id}
            review={selectedSuccessReview}
            onClose={() => setSelectedSuccessReview(null)}
          />
        )}
      </AnimatePresence>
      <StorybookFooter
        onReplayIntro={replayBookIntro}
      />
    </div>
  );
};
