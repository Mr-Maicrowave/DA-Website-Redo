...
export const EnglishIntroVideoGate = () => {
  const [isOpen, setIsOpen] = useState(shouldShowEnglishIntroOnThisAppLoad);
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="english-intro-video-title"
      onKeyDown={(event) => {
        if (event.key === 'Tab') event.preventDefault();
      }}
    >
      <h2 id="english-intro-video-title" className="sr-only">English introduction</h2>
      <video
        className="h-full w-full object-cover"
        src="/english_intro_video.mp4"
        autoPlay
        muted
        playsInline
        onEnded={dismiss}
