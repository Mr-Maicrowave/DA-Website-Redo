...
export const ScienceIntroVideoGate = () => {
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="science-intro-video-title"
      onKeyDown={(event) => {
        if (event.key === 'Tab') event.preventDefault();
      }}
    >
      <h2 id="science-intro-video-title" className="sr-only">Science introduction</h2>
      <video
        className="h-full w-full object-cover"
        src="/science_intro_video.mp4"
        autoPlay
        muted
        playsInline
        onEnded={dismiss}
