...
export const WayfinderMap = ({ selectedCentre, variant = 'hero' }: { selectedCentre: PhysicalCentre; variant?: WayfinderMapVariant }) => {
  ...
  const station = scene.places.find((place) => place.kind === 'station');
  ...
  return <svg className="wayfinder-map" viewBox={`0 0 ${scene.viewBox.width} ${scene.viewBox.height}`} preserveAspectRatio={variant === 'nearby' ? 'none' : undefined} aria-hidden="true" data-testid="wayfinder-map-geometry">
    <motion.g animate={camera} transition={{ duration: reduceMotion ? 0 : .8, ease: [0.22, 1, .36, 1] }}>
      ...
      {scene.roads.filter((road) => road.hierarchy === 'major').map((road) => {
        ...
        const isHeroLandmark = road.id === 'canley-vale-road' && variant === 'hero';
        const labelX = isHeroLandmark ? midpoint.x - 180 : midpoint.x + 12;
        return <text key={`${road.id}-label`} x={labelX} y={midpoint.y - (isHeroLandmark ? 24 : 10)} className={`wayfinder-label wayfinder-road-label wayfinder-road-label-${road.id}`}>{road.name}</text>;
      })}
      {scene.places.map((place) => {
        ...
        return <g key={place.id} className={`wayfinder-place wayfinder-place-${place.id}`}>
          {place.kind === 'station' && <g className="wayfinder-station-symbol" transform={`translate(${point.x} ${point.y})`}>
            <circle r="10" />
            <path d="M-3.5 -4.5v7M3.5 -4.5v7M-5.5 5h11" />
          </g>}
          <circle cx={point.x} cy={point.y} r="4" />
          {showLabel && <text x={point.x + (place.labelOffset?.x ?? 12)} y={point.y + (place.labelOffset?.y ?? -12)} className="wayfinder-label">{place.name}</text>}
        </g>;
      })}
      ...
      {PHYSICAL_CENTRES.map((centre) => {
        ...
        return <g key={centre.id} transform={`translate(${point.x} ${point.y})`} className={selected ? 'is-selected' : 'wayfinder-centre--secondary'} opacity={selected ? 1 : .58}>
          {selected && <circle r="34" className="wayfinder-marker-glow" />}
    ...
    {variant === 'hero' && station && <g className="wayfinder-station-cue" transform="translate(690 620)">
      <circle r="11" />
      <path className="wayfinder-station-cue__rail" d="M-3.5 -4.5v7M3.5 -4.5v7M-5.5 5h11" />
      <text x="-18" y="4" textAnchor="end">{station.name}</text>
      <path className="wayfinder-station-cue__direction" d="M16 2h42m-8-8 8 8-8 8" />
    </g>}
  </svg>;
};
