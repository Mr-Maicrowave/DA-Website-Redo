...
function interspersePullQuotes(
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if ((i + 1) % 3 === 0 && pullQuotes.length > 0) {
      elements.push(
        <PullQuote
          key={`pq-${i}`}
          text={pullQuotes[quoteIndex % pullQuotes.length].text}
        />
      );
      quoteIndex++;
    }
...
export default function StudentReviewLayout({ testimonial }: Props) {
  const { title, subtitle, label, bodyParagraphs, pullQuotes, calloutBoxes, bottomQuote, bottomAuthor } = testimonial;

  const featuredCallout = calloutBoxes[0] ?? null;
  const remainingCallouts = calloutBoxes.slice(1);

  return (
    <article className="max-w-5xl mx-auto">
      <div className="rounded-xl overflow-hidden">
        <div className="h-1.5 bg-gradient-to-r from-brand-navy to-teal-700 rounded-t-lg" />
            ...
            <aside className="space-y-4">
              {featuredCallout && (
                <div className="bg-stone-800 text-white p-6 rounded-lg">
                  <p className="uppercase tracking-wider text-xs font-semibold text-amber-400 mb-3">
                    {featuredCallout.header}
                  </p>
                  <p className="text-base font-bold leading-relaxed">{featuredCallout.content}</p>
                </div>
              )}
